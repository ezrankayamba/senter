package tz.co.nezatech.cusi.handler;
...
import tz.co.nezatech.cusi.data.model.Permission;
import tz.co.nezatech.cusi.data.model.Role;
import tz.co.nezatech.cusi.data.repository.PermissionRepository;
...
public class InitHandler implements ApplicationListener<ApplicationReadyEvent> {
	...
	static final String UI_CONTROLLERS_PACKAGE = "tz.co.nezatech.cusi.web.ui";
	...
	void initPermissions() {
		...
		Reflections ref = new Reflections(UI_CONTROLLERS_PACKAGE);
		...
		try {
			...
			for (Class<?> cl : ref.getTypesAnnotatedWith(PreAuthorize.class)) {
				String clsName = String.format("%s.%s", UI_CONTROLLERS_PACKAGE, cl.getSimpleName());
				logger.debug("Controller: " + cl.getSimpleName());
				logger.debug("Full name: " + clsName);
				...
				if (matcher.find() || strClsAuth.equalsIgnoreCase("permitAll")) {
					...
					Reflections refM = new Reflections(clsName, new MethodAnnotationsScanner());
